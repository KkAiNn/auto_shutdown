...
// Daily mode tracking state
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct DailyModeState {
    pub is_active: bool,
    pub daily_limit_minutes: u32,
    pub used_minutes_today: u32,
    pub last_recorded_date: String, // YYYY-MM-DD format
    pub force_shutdown_triggered: bool,
}

impl Default for DailyModeState {
    fn default() -> Self {
        Self {
            is_active: false,
            daily_limit_minutes: 480, // Default 8 hours
            used_minutes_today: 0,
            last_recorded_date: String::new(),
            force_shutdown_triggered: false,
        }
    }
}

pub static DAILY_MODE_STATE: Mutex<DailyModeState> = Mutex::new(DailyModeState {
    is_active: false,
...
// Get daily mode data file path
fn get_daily_mode_path(app_handle: &AppHandle) -> PathBuf {
    let app_dir = app_handle.path().app_data_dir().unwrap_or_else(|_| PathBuf::from("."));
    app_dir.join("daily_mode.json")
}

// Load daily mode data from file
fn load_daily_mode_data(app_handle: &AppHandle) -> DailyModeState {
    let path = get_daily_mode_path(app_handle);
    if path.exists() {
        if let Ok(content) = std::fs::read_to_string(&path) {
            if let Ok(data) = serde_json::from_str::<DailyModeState>(&content) {
                return data;
            }
        }
    }
    DailyModeState::default()
}

// Save daily mode data to file
fn save_daily_mode_data(app_handle: &AppHandle, state: &DailyModeState) -> Result<(), String> {
    let path = get_daily_mode_path(app_handle);
    let json = serde_json::to_string(state).map_err(|e| e.to_string())?;
    std::fs::write(&path, json).map_err(|e| e.to_string())
}

// Check and initialize daily mode on app start
fn check_and_init_daily_mode(app: &tauri::App) {
    let app_handle = app.handle().clone();
    // Load settings to check if daily mode is enabled
    let settings_path = if let Ok(app_dir) = app_handle.path().app_data_dir() {
        app_dir.join("settings.json")
    } else {
        return;
    };
    
    let (daily_mode_enabled, daily_limit) = if settings_path.exists() {
        if let Ok(content) = std::fs::read_to_string(&settings_path) {
            if let Ok(settings) = serde_json::from_str::<serde_json::Value>(&content) {
                let enabled = settings.get("dailyMode")
                    .and_then(|v| v.as_bool())
                    .unwrap_or(false);
                let limit = settings.get("dailyMinutes")
                    .and_then(|v| v.as_u64())
    ...
    if !daily_mode_enabled {
        return;
    }
    
    // Load previous daily mode data
    let mut daily_state = load_daily_mode_data(&app_handle);
    let today = chrono::Local::now().format("%Y-%m-%d").to_string();
    
    // Check if it's a new day
    if daily_state.last_recorded_date != today {
        daily_state.used_minutes_today = 0;
        daily_state.last_recorded_date = today.clone();
        daily_state.force_shutdown_triggered = false;
    }
    
    daily_state.is_active = true;
    daily_state.daily_limit_minutes = daily_limit;
    
    // Check if already exceeded limit
    if daily_state.used_minutes_today >= daily_state.daily_limit_minutes && !daily_state.force_shutdown_triggered {
        daily_state.force_shutdown_triggered = true;
        // Trigger immediate 1-minute force shutdown
        let _ = add_log_entry(&app_handle, "daily_mode", "每日时长已用完，触发强制关机", None);
        trigger_force_shutdown(app_handle.clone());
    }
    
    // Save updated state
    let _ = save_daily_mode_data(&app_handle, &daily_state);
    
    // Update global state
    if let Ok(mut state) = DAILY_MODE_STATE.lock() {
        *state = daily_state;
    }
    
    // Start background thread to track usage time
    let app_handle_clone = app_handle.clone();
    thread::spawn(move || {
        track_daily_usage(app_handle_clone);
    });
}

// Background thread to track daily usage
fn track_daily_usage(app_handle: AppHandle) {
    ...
    loop {
        ...
        if let Ok(mut state) = DAILY_MODE_STATE.lock() {
            ...
            if state.last_recorded_date != today {
                ...
                state.force_shutdown_triggered = false;
            }
            
            // Increment used time
            state.used_minutes_today += 1;
            
            // Check if limit reached
            let remaining = state.daily_limit_minutes.saturating_sub(state.used_minutes_today);
            
            if remaining == 1 && !state.force_shutdown_triggered {
                // 1 minute warning - show notification
                let _ = add_log_entry(&app_handle, "daily_mode", "每日时长即将用完，1分钟后强制关机", None);
                // TODO: Send system notification
            }
            
            let should_trigger_shutdown = state.used_minutes_today >= state.daily_limit_minutes && !state.force_shutdown_triggered;
            let should_save = last_save.elapsed().as_secs() >= 300 || should_trigger_shutdown;
            
            if should_trigger_shutdown {
                state.force_shutdown_triggered = true;
                let state_clone = state.clone();
                let _ = save_daily_mode_data(&app_handle, &state_clone);
                drop(state); // Release lock before triggering shutdown
                
                let _ = add_log_entry(&app_handle, "daily_mode", "每日时长已用完，开始强制关机倒计时", None);
                trigger_force_shutdown(app_handle.clone());
            } else if should_save {
                let state_clone = state.clone();
                let _ = save_daily_mode_data(&app_handle, &state_clone);
...
// Trigger force shutdown with 1-minute countdown
fn trigger_force_shutdown(app_handle: AppHandle) {
    thread::spawn(move || {
        // 1 minute countdown
        let countdown_seconds = 60;
        
        // Log the warning
        let _ = add_log_entry(&app_handle, "daily_mode", "强制关机倒计时开始：60秒", None);
        
        // Wait 1 minute
        thread::sleep(Duration::from_secs(countdown_seconds as u64));
        
        // Execute force shutdown - cannot be stopped by task manager
        #[cfg(not(debug_assertions))]
        {
            let _ = add_log_entry(&app_handle, "daily_mode", "执行强制关机", None);
            
            // Use shutdown with /f (force) flag - applications are forced to close
            // This shutdown cannot be easily stopped by task manager
        ...
        {
            let _ = add_log_entry(&app_handle, "daily_mode", "[开发模式] 跳过强制关机", None);
            println!("[开发模式] 每日时长限制触发，跳过强制关机");
        }
    });
}

// Get daily mode status
#[tauri::command]
fn get_daily_mode_status() -> (bool, u32, u32, u32) {
    if let Ok(state) = DAILY_MODE_STATE.lock() {
        let remaining = state.daily_limit_minutes.saturating_sub(state.used_minutes_today);
        (state.is_active, state.daily_limit_minutes, state.used_minutes_today, remaining)
    } else {
        (false, 0, 0, 0)
    }
}
